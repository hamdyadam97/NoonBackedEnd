...
namespace None.Infrastructure
{
    public class CategoryRepository : Repoditory<Category, int>, ICategoryRepository
    {
        ...
        public async Task<IEnumerable<Product>> getProductByNameCategory(string Name, int num)
        {
            var products = await _context.Products.Include(p => p.Images)
            .Where(p => p.ProductCategories.Any(pc => pc.Category.Name.ToLower() == Name.ToLower())).Take(num)
            .ToListAsync();
            return products;
        }
    }
