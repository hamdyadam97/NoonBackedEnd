...
namespace AliExpress.Api.Controllers
{
    ...
    public class CategoryController : ControllerBase
    {
        ...
        [HttpGet("GetAllProductsByCategoryName/{cateName}/{num}")]
        public async Task<ActionResult<IEnumerable<ProductViewDto>>> GetAllProductByCategoryName(string cateName, int num)
        {
            var products = await _categoryService.GetAllProductsByNameCategory(cateName, num);
            if (products == null)
            {
                return NotFound();
            }
