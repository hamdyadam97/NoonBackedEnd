...
namespace AliExpress.Application.Services
{
    public class OrderService :IOrderService
    {
        ...
        public async  Task<IEnumerable<OrderReturnDto>> GetOrderByUserIdAsync(string userId)
        {
            var orders = await _orderRepository.GetByUserIdAsync(userId);
            var mappedOrder = _mapper.Map<IEnumerable<Order>, IEnumerable<OrderReturnDto>>(orders); ;
            
            return mappedOrder;
        }

        public async Task<OrderReturnDto> GetOrderLastAsync(string userId)
        {
            var order = await _orderRepository.GetOrderLastAsync(userId);
            var mappedOrder = _mapper.Map<Order, OrderReturnDto>(order);
            return mappedOrder;
        }
